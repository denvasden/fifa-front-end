...
const { getOneAbstraction, parseCountry, parseLeague, parseTeam } = require("./utilities")

const COUNTRIES_WITHOUT_LEAGUE = ["WOMEN'S NATIONAL", "REST OF WORLD", "MEN'S NATIONAL"]

function parseText(textData, substrStart = "<h2>ARGENTINA</h2", substrEnd = "<p>Wilstermann</p>") {
...
function parseBlock(textBlock, dataFIFA) {
   ...
   contriesLeaguesAndTeams.forEach(line => {
      ...
      league && dataFIFA.leagues.push({ "country": dataFIFA.countries[dataFIFA.countries.length - 1], "league": league });
      team && dataFIFA.teams.push({ ...dataFIFA.leagues[dataFIFA.leagues.length - 1], "team": team });
...
function parseFIFA(textData, substrStart = "<h2>ARGENTINA</h2>", substrEnd = "<p>Wilstermann</p>") {

   const indexIncludesSubstrStart = textData.indexOf(substrStart);
   const indexIncludesSubstrEnd = textData.lastIndexOf(substrEnd);

   const coreText = textData.slice(indexIncludesSubstrStart, indexIncludesSubstrEnd) + substrEnd;
